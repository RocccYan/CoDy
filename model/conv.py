...
class HGTConv(MessagePassing):
    r"""
    Refining by adding RTE.
    Original implementation of the Heterogeneous Graph Transformer from PyG:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.use_RTE:
            self.temporal_enc = RelTemporalEncoding(
                in_channels[self.node_types[0]] if isinstance(in_channels,dict) else in_channels,
                max_len=time_span)
    ...
    def forward(
        ...
        x_time_dict: Optional[Dict[NodeType, Tensor]] = None,
        ...
        :rtype: :obj:`Dict[str, Optional[torch.Tensor]]` - The output node
            ...
            be set to :obj:`None`.
        """
        # RTE, just for source node(paper), we only got paper's published timestamp:
        if self.use_RTE and x_time_dict is not None:
            for node_type, node_time in x_time_dict.items():
                # src_type, _, _ = edge_type
                # Apply RTE to the features of source nodes of each edge type.
                # You're updating x_dict in-place with temporally encoded features.
                print(edge_type,"RTE is applied...")
                x_dict[src_type] = self.temporal_enc(x_dict[src_type], edge_time)
