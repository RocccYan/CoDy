...
from model.model import HGT, CitationPredictor, RGAT
...
def main(data_args, model_args, training_args):
    ...
    # num_samples = [5]
    # TODO. check data in batch, num of rev_cite edges.
    num_samples = [training_args.num_samples] * model_args.num_layers
    batch_size = training_args.batch_size

    # randomly initialize the edge time
    edge_time_dict = {}
    for edge_type in train_data.edge_types:
        edge_time_dict[edge_type] = torch.randint(0, 10, (train_data[edge_type].edge_index.shape[1],))
    train_data.edge_time_dict = edge_time_dict
    # logger.info(edge_time_dict)
        

    sampler = NeighborLoader if training_args.sampler == 'neighbor' else HGTLoader
    ...
    if model_args.model_name == 'hgt':
        encoder = HGT(
            hidden_channels=model_args.hidden_channels, 
            num_heads=model_args.num_heads, 
            num_layers=model_args.num_layers,
            data=train_data,
            use_RTE=model_args.use_RTE,)
    else:
        encoder = RGAT(
            author_features=768,
            paper_features=768,
            venue_features=768,
            out_channels=model_args.hidden_channels,
        )
    model = CitationPredictor(
        in_channels=model_args.hidden_channels, 
        num_classes=data_args.num_classes)
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    logger.info(f'Using device: {device}')
    train_data, encoder, model = train_data.to(device), encoder.to(device), model.to(device)
    ...
    with torch.no_grad():
        ...
        _ = encoder(batch.x_dict, batch.edge_index_dict)
        # logger.info(out)
    # os._exit(-1)
    ...
    for epoch in range(0, training_args.num_epochs):
        ...
        loss_reg,loss_cls,acc,f1 = train(train_loader,encoder=encoder,model=model,loss_fn=loss_fn,
                                         optimizer=optimizer,target=target,logger=logger)
